...
def readHTK(file_path, big_endian=True, scale_s_rate=True):
    """
    ...
    - scale_s_rate  (Boolean) - Puts sampling rate in Hz (native is 10000)
...
def toHTK(file_data, file_path, big_endian=True, scale_s_rate=False):
    """
...
def htk2wav(htkpath, wavpath=None):
    """
    Converts HTK file to .wav. If wav path is not given, it automatically
    assigns one using the lab convention of 'ANIN#.htk' -> 'analog#.wav'

    Parameters
    ----------
    htkpath: str or path
    wavpath: None or str or path
    """
    audiopath = os.path.split(htkpath)[0]

    HTKout = readHTK(htkpath)
    data = HTKout['data'].ravel()
    fs = int(HTKout['sampling_rate'])

    if wavpath is None:
        num = str(htkpath)[-5]
        wavpath = os.path.join(audiopath, 'analog' + num + '.wav')

    # scale
    scaled = np.int16(data / np.max(np.abs(data)) * 32767)
    wavwrite(wavpath, fs, scaled)
